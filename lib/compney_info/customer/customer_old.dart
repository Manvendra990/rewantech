...
//   // ✅ Form + per-field keys used to validate and auto-scroll to the first
//   // missing required field on save.
//   final _formKey = GlobalKey<FormState>();
//   final _ownerNameFieldKey = GlobalKey();
//   final _businessNameFieldKey = GlobalKey();
//   final _phoneFieldKey = GlobalKey();
//   final _agentNameFieldKey = GlobalKey();
//   final _totalAmountFieldKey = GlobalKey();
//   final _monthlyAmountFieldKey = GlobalKey();
//   final _serviceChargesFieldKey = GlobalKey();
//   final _amountReceivedFieldKey = GlobalKey();
//   final _incentivePercentFieldKey = GlobalKey();
//   final _incentiveAmountFieldKey = GlobalKey();
//   final _platformFieldKey = GlobalKey();
//   final _socialIdFieldKey = GlobalKey();
//   final _socialPasswordFieldKey = GlobalKey();
//   final _customServiceFieldKey = GlobalKey();
//   final _monthlyTasksFieldKey = GlobalKey();

//   // ✅ Validation only turns "live" (revalidating on every keystroke) AFTER
//   // the first failed Save to Cloud attempt. Before that, fields stay quiet
//   // while the user is still filling the form in — errors only appear once
//   // Save is tapped and something's missing, per _saveToCloud() below.
...
//   // Used for numeric fields that are optional overall (agent
//   // incentive/deal/service amounts) — an empty field is fine, but if the
//   // user did type something it still has to be a valid number.
...
//   // This service's own monthly charges — intentionally separate from
//   // _monthlyAmountController (which is the deal-level billing amount and
//   // only shows up when Payment Type == 'Monthly'). This field is always
//   // visible so Add Service works regardless of that toggle.
...
//   // ---------------------------------------------------------------------
//   // Validates EVERY required field directly from its controller's raw
//   // text — NOT via Form/FormField — in the SAME order the fields appear
//   // on screen. Returns the GlobalKey of the first invalid field found (or
//   // null if everything's valid) so the caller can scroll/focus it.
//   //
//   // Why this exists instead of trusting Form.validate() alone: this form
//   // is long and lives inside a scrolling ListView. Flutter's ListView
//   // lazily disposes widgets that scroll off-screen, and a disposed
//   // TextFormField unregisters itself from its ancestor Form. Since users
//   // must scroll down to reach the Save button, top fields (Owner Name,
//   // Business Name, etc.) can end up off-screen and disposed by the time
//   // Save is tapped — meaning Form.validate() would only be checking
//   // whatever happens to still be mounted, and could report "valid" even
//   // though required fields are empty. Reading straight from the
//   // controllers here sidesteps that entirely: it doesn't matter whether
//   // the field widget is currently mounted, its value is still checked.
//   // ---------------------------------------------------------------------
//   GlobalKey? _firstInvalidFieldKey() {
//     if (_requiredValidator(_ownerNameController.text) != null) {
//       return _ownerNameFieldKey;
//     }
//     if (_requiredValidator(_businessNameController.text) != null) {
//       return _businessNameFieldKey;
//     }
//     if (_requiredValidator(_phoneController.text) != null) {
//       return _phoneFieldKey;
//     }
//     if (_requiredValidator(_agentNameController.text) != null) {
//       return _agentNameFieldKey;
//     }
//     if (_requiredNumberValidator(_incentivePercentController.text) != null) {
//       return _incentivePercentFieldKey;
//     }
//     if (_requiredNumberValidator(_incentiveAmountController.text) != null) {
//       return _incentiveAmountFieldKey;
//     }
//     if (_requiredValidator(_platformController.text) != null) {
//       return _platformFieldKey;
//     }
//     if (_requiredValidator(_socialIdController.text) != null) {
//       return _socialIdFieldKey;
//     }
//     if (_requiredValidator(_socialPasswordController.text) != null) {
//       return _socialPasswordFieldKey;
//     }
//     if (_paymentType == 'One-time') {
//       if (_requiredNumberValidator(_totalAmountController.text) != null) {
//         return _totalAmountFieldKey;
//       }
//     }
//     if (_requiredNumberValidator(_amountReceivedController.text) != null) {
//       return _amountReceivedFieldKey;
//     }
//     if (_serviceProvidedName == 'Other' &&
//         _requiredValidator(_customServiceProvidedController.text) != null) {
//       return _customServiceFieldKey;
//     }
//     if (_requiredNumberValidator(_serviceChargesController.text) != null) {
//       return _serviceChargesFieldKey;
//     }
//     if (_requiredValidator(_monthlyTasksController.text) != null) {
//       return _monthlyTasksFieldKey;
//     }
//     if (_paymentType == 'Monthly') {
//       if (_requiredNumberValidator(_monthlyAmountController.text) != null) {
//         return _monthlyAmountFieldKey;
//       }
//     }
//     return null;
//   }
...
//     // Errors are intentionally silent up to this point (Form's
//     // autovalidateMode starts as `disabled`, see the field above) — the
//     // user shouldn't see "required" text while they're still filling the
//     // form in. Tapping Save is what triggers validation for the first
//     // time.
//     //
//     // The form's scroll container is a SingleChildScrollView + Column
//     // (see build() below), NOT a ListView — every field is built once and
//     // stays mounted for the sheet's whole lifetime, regardless of scroll
//     // position. That means Form.validate() below correctly checks EVERY
//     // required field (not just whichever ones happen to be on screen
//     // right now) and renders its "required" error text under each one,
//     // and _scrollToField() can always find a valid context to scroll to
//     // for any field, including ones near the very top.
//     //
//     // `_firstInvalidFieldKey()` reads straight from the controllers as a
//     // second, independent check — belt-and-suspenders in case Form.validate()
//     // and this ever disagree — and is what actually decides whether the
//     // save proceeds.
//     _formKey.currentState?.validate();
//     final firstInvalidKey = _firstInvalidFieldKey();

//     if (firstInvalidKey != null) {
//       // From here on, re-validate live as the user edits, so error text
//       // clears field-by-field as they fix things instead of staying
//       // frozen until the next Save tap.
...
//               // IMPORTANT: this is a SingleChildScrollView + Column, not a
//               // ListView. A ListView virtualizes its children — it disposes
//               // widgets that scroll off-screen to save memory, and a
//               // disposed TextFormField unregisters itself from this Form.
//               // Since reaching the Save button requires scrolling past
//               // fields near the top (Owner Name, Business Name, etc.),
//               // those fields would get disposed by the time Save is
//               // tapped — meaning Form.validate() couldn't see them AND
//               // Scrollable.ensureVisible couldn't scroll back up to them
//               // (GlobalKey.currentContext is null for anything not
//               // currently mounted). SingleChildScrollView + Column builds
//               // every field once and keeps them all mounted for the
//               // lifetime of the sheet, which fixes both problems at the
//               // root instead of working around them.
...
//                   // This service's own monthly charges — always visible,
//                   // independent of the deal-level One-time/Monthly toggle
//                   // above (that's what "Monthly Payment Amount" controls).
//                   TextFormField(
//                     key: _serviceChargesFieldKey,
